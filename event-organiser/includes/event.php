...
*/
	function eo_update_event($post_id, $event_data=array(), $post_data=array() ){
		...
		$delete_existing = ($event_data['schedule'] == 'irregular') ? true : false;
		$diff = array();
		if( $prev ){
		...
		if( $delete_existing ){
			...
			//return new WP_Error('eo_error',serialize($event_data['occurrences_end']));
	...
	function  _eventorganiser_insert_occurrences($post_id, $event_data){
		...
		foreach( $occurrences as $counter=> $occurrence ):
			if(empty($occurrence_ends)){
				...
				$occurrence_end = $occurrence_ends[$counter];
			}
...
*/
	function _eventorganiser_generate_occurrences( $event_data=array() ){
		...
		//White list schedule
		if( !in_array($schedule, array('once','daily','weekly','monthly','yearly','custom','irregular')) )
		...
		switch( $schedule ) :
			...
			case 'yearly':
				...
				break;
			case 'irregular':
				//check whether we actually have recurrences scheduled
				if(empty($irregular))
					return new WP_Error('eo_error',__('No occurences scheduled','eventorganiser'));
				...
				foreach($irregular['start'] as $index=>$starts) if(!empty($starts)):
					$startDateTime = _eventorganiser_check_datetime($starts, false);
					
					//_check_datetime will return false on parse errors. Don't add those
					if($startDateTime !== false) $start_days[] = $startDateTime;
				endif;
				
				foreach($irregular['end'] as $ends) if(!empty($ends)):
					$endDateTime = _eventorganiser_check_datetime($ends, false);
					
					//_check_datetime will return false on parse errors. Don't add those
					if($endDateTime !== false):
						$end_days[] = $endDateTime;
					else:
		...
		foreach($start_days as $index => $start_day):
			...
			switch($workaround):
				...
				case 'irregular':
					$occurrences[] = clone $current;
					$occurrence_ends[] = clone $end_days[$index];
					break;
		...
		$occurrence_ends = _eventorganiser_remove_duplicates($occurrence_ends);

		//Sort occurrences
		sort($occurrences);
		sort($occurrence_ends);
