	...
	function parse_template($matches){
		...
		switch($matches[1]):
			...
			case 'event_location':
				...
				break;
			case 'event_datespan':
				$start = eo_get_the_start('g:ia l j F Y');
				$end = eo_get_the_end('g:ia l j F Y');
				$start_split = explode(' ',$start);
				$end_split = explode(' ',$end);
				if($start == $end){
					$replacement = $start_split[0];
				}elseif(
					$start_split[4] == $end_split[4]
					&& $start_split[3] == $end_split[3]
					...
					if(eo_is_all_day()){
						...
						$replacement = $start_split[0]."-".$end_split[0];
					}
				}elseif(
					$start_split[4] == $end_split[4]
					&& $start_split[3] == $end_split[3]
					...
					$replacement = sprintf('%s %s, %s - %s %s, %s',
						$start_split[3],
						$start_split[2],
						$start_split[4],
						$end_split[3],
